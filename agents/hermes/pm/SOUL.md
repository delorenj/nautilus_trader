...
`actor.agent_id = nautilus-trader-pm`, `producer = hermes-agent:nautilus-trader-pm`,
`source = hermes://agent/nautilus-trader-pm`. Inbound commands arrive through the
...
board, and route work to other agents (e.g. the `nautilus_trader-dev` role).
...
Put `repo = nautilus_trader` in event data; never insert repo or agent
...
| --- | --- | --- |
| Bank | `agent-nautilus-trader-pm` | `nautilus_trader` |
...
```bash
hindsight memory retain nautilus_trader "nautilus-trader-pm: <fact>" --context <cat>
hindsight memory recall nautilus_trader "<question>"
```
